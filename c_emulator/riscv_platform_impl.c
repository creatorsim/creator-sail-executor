...
uint32_t check_cache = 0;
uint32_t crep_value = 0;
uint8_t kernel = 1;
uint8_t sudo = 0;
uint32_t creator_entry = 0;
bool rv_enable_dirty_update = false;
bool rv_enable_misaligned = false;
bool rv_mtval_has_illegal_inst_bits = false;
bool rv_enable_writable_fiom = true;

/*PAra la version de 32bits*/
#ifdef RV32
  uint64_t rv_rom_base = UINT64_C(0x00001000);
  uint64_t rv_rom_size = UINT64_C(0xC000);

  uint64_t rv_ram_base = UINT64_C(0x80000000);
  uint64_t rv_ram_size = UINT64_C(0xA000);
#endif

/* Para la version de 64 bits */

#ifdef RV64

  uint64_t rv_ram_base = UINT64_C(0x00000000);
  uint64_t rv_ram_size = UINT64_C(0x40000000);

  uint64_t rv_rom_base = UINT64_C(0xFFFFFFFF80000000);
  uint64_t rv_rom_size = UINT64_C(0x80000000);
