...
// #define WRITE_TOHOST_DEFAULT 
//     0x00000f17u, /* auipc t5, 0x0 */ 
//     0x128f3f03u, /* ld t5, 0x128(t5) <tohost> */ 
//     0x003f2023u, /* sw gp, 0x0(t5) */ 
//     0x000f2223u, /* sw zero, 0x4(t5) */ 
//     0xff1ff06fu  /* j write_tohost */

// #define ENABLE_FP_EXTENSION 
//     0x00002537u, /* li a0, 0x2 */ 
//     0x30052073u, /* csrs mstatus, a0 */ 
//     0x00305073u, /* csrwi fcsr, 0 */ 
//     0x000022b7u, /* lui t0, 0x2  */ 
//     0x8002829bu, /* addiw t0, t0, -2048 */ 
//     0x3002b073u /* csrrc zero, mstatus, t0 */ 

// #define ENABLE_VECTOR_EXTENSION 
//     0x02200513u, /* li a0, 34 */ 
//     0x30052073u, /* csrs mstatus, a0 */ 
//     0x00305073u, /* csrwi fcsr, 0 */ 
//     0x00f05073u, /* csrwi vcsr, 0 */ 
//     0x000022b7u, /* lui t0, 0x2  */ 
//     0x8002829bu, /* addiw t0, t0, -2048 */ 
//     0x3002b073u /* csrrc zero, mstatus, t0 */ 
...
// #define WRITE_TOHOST_VECTOR 
//     0x00000f17u, /* auipc t5, 0x0 */ 
//     0x138f3f03u, /* ld t5, 0x138(t5) <tohost> */ 
//     0x003f2023u, /* sw gp, 0x0(t5) */ 
//     0x000f2223u, /* sw zero, 0x4(t5) */ 
//     0xff1ff06fu  /* j write_tohost */
...
size_t dtb_len = 0;
uint8_t flags = 0;  
// uint64_t creator_entry;
/*
Los flags mas importantes que son necesarios configurar son para coma flotante y para vectoriales.
El resto son complementarios y sirven para descartar ejecuciones en caso de que generen conflicto
*/
...
// }
// bool is_fp(){
//   return ((flags >> 3) & 1) | ((flags >> 4) & 1);
// }

// bool check_bit(){
//   fprintf(stderr, "Valor: %d\n", flags);
//   if(((flags >> 0) & 1) != 0 && (((flags >> 1) & 1) | ((flags >> 2) & 1) ) != 0 ){
//     return 0;
//   }
//   else if(((flags >> 0) & 1) != 0 )
//     rv_enable_bext = true;
...
// bool check_vector(){
//   fprintf(stderr, "Valor: %d\n", flags);
//   if(((flags >> 2) & 1) != 0 && (((flags >> 3) & 1) || ((flags >> 4) & 1) || ((flags >> 5) & 1)) != 0 ){
//     return 0;
//   }
//   else if (((flags >> 2) & 1) != 0){
//   }
...
// bool check_fp(){
//   if(((flags >> 2) & 1) != 0 && (((flags >> 3) & 1) | ((flags >> 4) & 1)) != 0 )
//     return 0;
//   else if((((flags >> 3) & 1) | ((flags >> 4) & 1)) != 0){
//     return 1;
//   }
//     return 1;
// }

// uint32_t calculate_entry_elf(uint32_t e){

//   uint32_t custom_entry = get_entry();
//   return custom_entry;
...
static struct option options[] = {
    {"cache-pol",                   required_argument, 0, 'o'                     },
    {"cache",                       required_argument, 0, 'y'                     },
    {"sudo",                        no_argument,       0, 'u'                     },
    {"kernel",                      no_argument,       0, 'k'                     },
    {"entry-address",               required_argument, 0, 'e'                     },
    // {"config-flags",                required_argument, 0, 'f'                     },
...
static int process_args(int argc, char **argv)
{
  ...
  while (true) {
    ...
    switch (c) {
    case 'o':
      sscanf(optarg, "%d", &crep_value);
      fprintf(stderr, "Memory cache replace policy: %d\n", crep_value);
      break;
    case'y':
      sscanf(optarg, "%d", &check_cache);
      fprintf(stderr, "Memory cache type: %d\n", check_cache);
      break;
    case'k':
      kernel = 1;
      break;
    case'u':
      ...
      break;
    case 'e':
      fprintf(stderr, "custom entry point\n");
      sscanf(optarg, "%x", &creator_entry);
      fprintf(stderr, "Cusmtom entry asigned as: %x\n", creator_entry);
      break;
    // case 'f':
    //   fprintf(stderr,"enabling additional extensions.\n");
    //   flags = (uint8_t) atoi(optarg);
    //   if (!(check_bit() && check_vector() && check_fp())){
    //     fprintf(stderr, "Your code has conflict with extensions available in riscv!\n");
    //     exit(1);
    //   }
    //   else
    //     fprintf(stderr,"No generan conflicto tus instrucciones en el codigo\n");
    //   break;
    case 'a':
      report_arch();
      break;
...
{
#ifdef RV32
  uint64_t tohost = 0x80006000;
#else
  uint64_t tohost = 0x0000000010000000;
#endif

#define RST_VEC_SIZE 140
  uint32_t reset_vec_nk[RST_VEC_SIZE] = {
        0x297,                              // auipc  t0,0x0
        ...
        /*10*/ is_32bit_model() ? entry :
         (uint32_t) (entry >> 32), 
         (uint32_t)(tohost & 0xffffffff),
         is_32bit_model() ? tohost :
        /*3 */ (uint32_t)(tohost >> 32)
  };

  uint32_t reset_vec_k[RST_VEC_SIZE] = {
        0x08c0006fu, // j reset_vector
        0x00d00293u, // syscall_write: li t0, 13
        0x0258f863u, // bgeu a7, t0, <fail>
        0xffd28293u, // addi t0, t0, -3
        0x01128a63u, // beq t0, a7, <exit>
        0x341022f3u, // csrr t0, mepc (para otro tipo de interrupcion printf/scanf)
        0x00428293u, // addi t0,t0,4
        0x34129073u, // csrw mepc,t0
        0x30200073u, // mret
        0x0ff0000fu, // exit: fence
        0x00100193u, // li gp, 1
        0x00000513u, // li a0, 0
        0x00a00893u, // li a7, 10
        0x0440006fu,//0x0440006fu,// 0x00000073u, // ecall salto a write_tohost jal pc + 0x44
        0x0ff0000fu, // fail: fence
        0x00018063u, // beqz gp, fail+0x4
        0x00119193u, // slli gp, gp, 0x1
        0x0011e193u, // ori gp, gp, 1
        0x00a00893u, // li a7, 10
        0x00018513u, // mv a0, gp
        0x0260006fu, // j handle_exception
        0x34202f73u, // trap_vector: csrr t5,mcause
        0x00800f93u, // li t6,8
        0xfbff04e3u, // beq t5, t6, <syscall_creator>
        0x00900f93u, // li t6,9
        0xfbff00e3u, // beq t5, t6, <syscall_creator>
        0x00b00f93u, // li t6, 11
        0xf9ff0ce3u,//0xfbff06e3u, // beq t5, t6, <syscall_creator>
        0x0040006fu, // j handle_exception
        0x5391e193u, // handle_exception: ori gp, gp, 1337
        0x00000f17u, /* auipc t5, 0x0 */ 
        (!rv_enable_fdext &&  rv_enable_vext  && !is_32bit_model()) ? 0x170f3f03u : 
        (!rv_enable_fdext &&  rv_enable_vext  && is_32bit_model())  ? 0x170f2f03u :
        (rv_enable_fdext  && !rv_enable_vext && !is_32bit_model()) ? 0x160f3f03u : 
        (rv_enable_fdext  && !rv_enable_vext && is_32bit_model())  ? 0x160f2f03u :
        (rv_enable_fdext  &&  rv_enable_vext  && !is_32bit_model()) ? 0x178f3f03u : // como 1er caso pero +3 
        (rv_enable_fdext  &&  rv_enable_vext  && is_32bit_model())  ? 0x17cf2f03u : // como el 2º caso pero +3
        is_32bit_model() ? 0x154f2f03u :
         0x150f3f03u,  /* ld t5, 0x128(t5) <tohost> */ 
        0x003f2023u, /* sw gp, 0x0(t5) */ 
        0x000f2223u, /* sw zero, 0x4(t5) */ 
        is_32bit_model() ? 0xff1ff06fu : 0xfedff06fu,  /* j write_tohost */
        0x00000093u, // reset_vector: li ra, 0
        is_32bit_model() ? 0x80007137u : 0x0010029bu, // 0x00000113u, // li sp, 0 añadir 
        is_32bit_model() ? 0xffc10113u : 0x01e29293u, /* añadir version 64bits */  // addi sp, sp, -4
        is_32bit_model() ? 0x00000193u : 0xffc28113u, 
        0x00000193u, // li gp, 0
        0x00000213u, // li tp, 0
        0x00000293u, // li t0, 0
        0x00000313u, // li t1, 0
        ...
        0x00000c13u, // li s8, 0
        0x00000c93u, // li s9, 0
        0x00000d13u, // li s10, 0
        0x00000d93u, // li s11, 0
        0x00000e13u, // li t3, 0
        0x00000e93u, // li t4, 0
        0x00000f13u, // li t5, 0
        0x00000f93u, // li t6, 0

        0xf1402573u, // csrr a0,mhartid
        0xfe051ee3u, // bnez a0, <reset_vector+0x7c (csrr a0,mhartid)>
        
        0x00000297u, // auipc t0,0x0 
        0x01028293u, // addi t0,t0,16 
        0x30529073u, // csrw mtvec,t0 
        0x30045073u, // csrwi mstatus,8
        
        0x00000297u, // auipc t0, 0x0 
        0x01028293u, // addi t0, t0, 16  
        0x30529073u, // csrw mtvec,t0  
        0x18005073u, // csrwi satp, 0
        
        0x00000297u, // auipc t0, 0x0
        0x02428293u, // addi t0,t0,36
        0x30529073u, // csrw mtvec, t0
        0x00100293u, // addi t0,zero, 1
        is_32bit_model() ? 0x01f29293u // slli t0,t0, 0x1f
         : 0x03529293u, // slli t0, t0, 0x35
        0xfff28293u, // addi t0,t0,-1
        0x3b029073u, // csrw pmpaddr0, t0
        0x01f00293u, // li t0,31
        0x3a029073u, // csrwi pmpcfg, t0
        
        0x30405073u, // csrwi mie, 0
        0x00000297u, // auipc t0, 0x0
        0x01428293u, // addi t0,t0,20
        0x30529073u, // csrw mtvec, t0
        0x30205073u, // csrwi medeleg, 0
        0x30305073u, // csrwi mideleg, 0
        
        0x00000297u, // auipc t0, 0x0
        is_32bit_model() ?  /*0xee428293u*/ 0xee028293u : 0xef028293u, // addi t0, t0, -284 <trap_vector> // Esto es en caso de que no este habilitada ninguna flag
        0x30529073u, // csrw mtvec, t0
        0x34129073u, // csrw mepc, t0
        // fp o double enabled?
        0x000022b7u, /* lui t0, 0x2  */ 
        is_32bit_model() ? 0x80028293u // addi t0, t0, -2048
        : 0x8002829bu, /* addiw t0, t0, -2048 */ 
        0x3002b073u, /* csrrc zero, mstatus, t0 */ 
        // cambiar mpp a 00 para modo usuario
        0x300022f3u, // csrr t0, mstatus
        0xffffe337u, // lui t1, 0xffffe
        0x7ff30313u, // addi t1, t1, 2047  => 0xffffe7ff
        0x0062f2b3u, // and t0, t0, t1
        is_machine_exec() ? 0x00300313u : 0x00000313u, // li t1, 0
        0x00b31313u,// slli t1, t1, 11
        0x0062e2b3u, // or t0, t0, t1 
        0x30029073u, // csrw mstatus, t0 => Write mstatus with mpp = 00 (user mode) or mpp = 11 (machine mode)


        0x00000297u, // auipc  t0,0x0
        (((rv_enable_fdext && !rv_enable_vext) || (!rv_enable_fdext && rv_enable_vext)) && !is_32bit_model()) ? 0x02028293u : // 0x0142b283u : //0x0142b283u : 
        ((rv_enable_vext || rv_enable_fdext) && is_32bit_model()) ? 0x02028293u : // 0x0202a283u : 
        is_32bit_model() ? 0x02028293u : /*0x0202a283u : */
        0x02028293u, // 0x0102b283u,  //0x0102b283u,
        // is_32bit_model() ? 0x0142a283u :    // lw     t0,16(t0)
        //     0x0142b283u,                    // ld     t0,16(t0)
        // por defecto 0x0102b283u
        0x34129073u, // csrw mepc, t0 
        0x30200073u, //  mret
        (uint32_t)(entry & 0xffffffff),     // 0x000011F4
        is_32bit_model() ? entry :
         (uint32_t) (entry >> 32), 
        (uint32_t)(tohost & 0xffffffff),
        is_32bit_model() ? tohost :
        (uint32_t)(tohost >> 32),
        /*is_32bit_model() ? */ get_entry(0),//0x800002b7u : 0x00000293u, // _init_main: call _main TODO: update to calculate the init add of program
        get_entry(1),
        0x000280e7u, 
        0x00a00893u, // li a7, 10
        0x00000073u  // ecall
  ...
  if(rv_enable_vext){
    memmove(&reset_vec_k[104], &reset_vec_k[97], 
    24 * sizeof(uint32_t));
    reset_vec_k[97] = 0x00002537u; // lui a0, 0x2
    reset_vec_k[98] = is_32bit_model() ? 0x20050513u : 0x2005051bu; // addiw a0, a0, 512
    reset_vec_k[99] = 0x30052073u; // csrs mstatus, a0
    reset_vec_k[100] = 0x02200513u; // li a0, 34
    reset_vec_k[101] = 0x30052073u; // csrs mstatus, a0
    /* Duplicado en dos lineas */reset_vec_k[102] = 0x00f05073u;//0x00305073u; // csrwi fcsr, 0
    reset_vec_k[103] = 0x00f05073u; // csrwi vcsr, 0
    // for (int i = 0; i < sizeof(reset_vec_k); i++){
    //   printf("Tras añadir vext Reset_vec_k[%d]: 0x%x\n", i, reset_vec_k[i]);
    // }
  }
  if(rv_enable_fdext){
    if (rv_enable_vext){
      memmove(&reset_vec_k[107], &reset_vec_k[104], 24 * sizeof(uint32_t));
      reset_vec_k[104] = 0x00002537u; // lui a0, 0x2
      reset_vec_k[105] = 0x30052073u; // csrs mstatus, a0
      reset_vec_k[106] = 0x00305073u; // csrwi fcsr, 0
    }else {
      memmove(&reset_vec_k[100], &reset_vec_k[97], 24 * sizeof(uint32_t));
      reset_vec_k[97] = 0x00002537u; // lui a0, 0x2
      reset_vec_k[98] = 0x30052073u; // csrs mstatus, a0
      reset_vec_k[99] = 0x00305073u; // csrwi fcsr, 0
    }
    // for (int i = 0; i < sizeof(reset_vec_k); i++){
    //   printf("Tras añadir fdext Reset_vec_k[%d]: 0x%x\n", i, reset_vec_k[i]);
    // }
  }
  if(!is_32bit_model() && ((rv_enable_fdext  && !rv_enable_vext) || (!rv_enable_fdext && rv_enable_vext))){
    if (rv_enable_fdext) {
      memmove(&reset_vec_k[116], &reset_vec_k[115], 
      9 * sizeof(uint32_t));
      reset_vec_k[115] = 0x0u;
    }
    else {
      memmove(&reset_vec_k[120], &reset_vec_k[119], 
      9 * sizeof(uint32_t));
      reset_vec_k[119] = 0x0u;
      
    }
  }
  // for (int i = 0; i < 140; i++){
  //   printf("Reset_vec_k[%d]: 0x%x\n", i, reset_vec_k[i]);
  // }

  rv_rom_base = DEFAULT_RSTVEC;
  uint64_t addr = rv_rom_base; 
  // calculate_entry_elf(0x00000000u);

  if(kernel_sim()) {
    for (int i = 0; i < sizeof(reset_vec_k); i++)
    write_mem(addr++, (uint64_t)((char *)reset_vec_k)[i]);
  }
  else {
    for (int i = 0; i < sizeof(reset_vec_nk); i++)
    write_mem(addr++, (uint64_t)((char *)reset_vec_nk)[i]);
